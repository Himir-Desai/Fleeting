...
///
/// Internal rather than private so ``GrowthProgress`` can draw it at a fixed width and reveal it
/// with a mask, which is what stops the leaves sliding apart as a session advances (ADR-0045).
struct Vine: Shape {
    let leaves: Int
